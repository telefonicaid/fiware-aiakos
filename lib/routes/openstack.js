...
function parseRegionNameFromUserName(username) {
    ...
    if ( username.indexOf('@') >= 0 ) {
        var error2 = new Error ('invalid username %s. Username can not be an email', username);
...
function getValidateToken(adminToken, token, callback) {
    ...
    req.on('error', function (e) {
        ...
        var error = new Error('Error in connection with openstack');
        error.statusCode = 401;
        throw error;
    ...
    req.on(
            "close",
            function handleCloseEvent(error) {
...
function validateTokenAndGetRegion(token, callback) {
    ...
    postAdminToken(function(adminToken) {
        ...
        getValidateToken(adminToken, token, function(result) {
            logger.debug('Region name, after validation token:' + result);
            callback(result);
