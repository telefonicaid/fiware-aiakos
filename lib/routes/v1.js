...
function regionsList(req, res) {
    ...
    var filter_region = req.query.region_name;
    ...
    if (filter_region === undefined) {
        ...
        files = files.filter(function (name) {
            return name.startsWith(filter_region);
        });
    ...
    files.forEach(function (item) {
        ...
        var region = {};
        if (regionsHashTable.has(data[0])) {
            region = regionsHashTable.get(data[0]);
            region[data[1]] = data[2];
            regionsHashTable.put(data[0], region);
