...
function readConfigFile(file) {
    ...
    try {
        ...
        ['app', 'logging', 'openstack', 'regions'].forEach(function (key) {
            switch (key in cfgParse && key) {
                ...
                case 'openstack':
                    ...
                    break;
                case 'regions':
                    Object.keys(config.regions).filter(hasOwnProperty, cfgParse.regions).forEach(function(key) {
                        config.regions[key] = cfgParse.regions[key];
                    });
                    break;
                default:
        ...
        var msg = err.message ? util.format('Configuration file: %s', err.message) :
            'Could not read configuration file';
        cfgParserResult = [ 'WARN', msg ];
    }

    // show result of configuration processing
    logger.info('config:' + cfgParserResult);
